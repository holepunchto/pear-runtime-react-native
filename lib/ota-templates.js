...
function swiftBundleUrl(bundleRoot) {
  return `override func bundleURL() -> URL? {
    ...
    let otaBundleURL = documentDirectory.appendingPathComponent("pear-runtime/upgrade/app.bundle")
...
function objCBundleUrl(bundleRoot) {
  ...
  NSURL *otaURL = [docDir URLByAppendingPathComponent:@"pear-runtime/upgrade/app.bundle"];
...
const ANDROID_GETJSBUNDLE_BODY = `      if (BuildConfig.DEBUG) return super.getJSBundleFile()
      val file = File(applicationContext.filesDir, "pear-runtime/upgrade/app.bundle")
